...
class FARWanAnyFlowPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        transformer: FAR_Wan_Transformer3DModel,
        vae: AutoencoderKLWan,
    ...
    def inference(
        ...
        if grad_timestep is None:
            latents = inference_range(latents, timesteps)
            return latents
        ...
        latents = inference_range(latents, current_timestep)
        latents = inference_range(latents, post_timestep)
        return latents
